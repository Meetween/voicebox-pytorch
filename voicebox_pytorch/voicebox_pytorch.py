...
class Attention(Module):
    def __init__(self, dim, dim_head=64, heads=8, dropout=0, flash=False, qk_norm=False, qk_norm_scale=10):
        super().__init__()
        self.heads = heads
            ...
            self.layers.append(
                nn.ModuleList(
                    [
                        ...
                        (
                            GateLoop(dim=dim, use_jax_associative_scan=gateloop_use_jax, post_ln=True)
                            if use_gateloop_layers
                            else None
                        ),
...
class MelVoco(AudioEncoderDecoder):
    ...
    def encode(self, audio):
        stft_transform = T.Spectrogram(
            n_fft=self.n_fft, win_length=self.win_length, hop_length=self.hop_length, window_fn=torch.hann_window
        )

        spectrogram = stft_transform(audio)

        mel_transform = T.MelScale(
            n_mels=self.n_mels, sample_rate=self.sampling_rate, n_stft=self.n_fft // 2 + 1, f_max=self.f_max
        )
    ...
    def decode(self, mel):
        ...
        if self.log:
            mel = DB_to_amplitude(mel, ref=1.0, power=0.5)

        return self.vocos.decode(mel)


class EncodecVoco(AudioEncoderDecoder):
    def __init__(self, *, sampling_rate=24000, pretrained_vocos_path="charactr/vocos-encodec-24khz", bandwidth_id=2):
        super().__init__()
        self.sampling_rate = sampling_rate
...
class DurationPredictor(Module):
    ...
    def __init__(
        ...
        *,
        ...
        frac_lengths_mask: Tuple[float, float] = (0.1, 1.0),
        ...
        assert not (
            exists(tokenizer) and exists(num_phoneme_tokens)
        ), "if a phoneme tokenizer was passed into duration module, number of phoneme tokens does not need to be specified"
    ...
    def forward_with_cond_scale(
        ...
        **kwargs,
        ...
        forward_kwargs = dict(return_aligned_phoneme_ids=False, phoneme_ids=phoneme_ids)

        durations = self.forward(*args, cond_drop_prob=0.0, **forward_kwargs, **kwargs)
        ...
        null_durations = self.forward(*args, cond_drop_prob=1.0, **forward_kwargs, **kwargs)
    ...
    def forward(
        ...
        *,
        ...
        if cond_drop_prob > 0.0:
            ...
            cond = torch.where(rearrange(cond_drop_mask, "... -> ... 1 1"), self.null_cond, cond)
        ...
        x = self.transformer(x, mask=self_attn_mask)
        ...
        assert all(
            [exists(el) for el in (phoneme_len, mel_len, phoneme_mask, mel_mask)]
        ), "need to pass phoneme_len, mel_len, phoneme_mask, mel_mask, to train duration predictor module"
...
class VoiceBox(Module):
    def __init__(
        ...
        *,
        ...
        assert not (
            condition_on_text and not exists(num_cond_tokens)
        ), "number of conditioning tokens must be specified (whether phonemes or semantic token ids) if training conditional voicebox"
    ...
    def forward(
        ...
        *,
        ...
        # FIXME resolve the following
        """
        x: noisy audio signal ?
        times: time steps for odeint
        cond_token_ids: phoneme or semantic token ids
        self_attn_mask: self attention mask in case of semantic token ids?
        cond_drop_prob: probability of dropping conditioning tokens
        target: target audio signal
        cond: conditioning signal
        cond_mask: mask for conditioning signal
        """
        ...
        if self.debug:
            ...
            # print(f"\nX shape: {x}")
            # print(f"Times: {times}")
            # print(f"Cond token ids shape: {cond_token_ids.shape}")
            # print(f"Self attn mask: {self_attn_mask}")
            # print(f"Cond drop prob: {cond_drop_prob}")
            # print(f"Target shape: {target.shape}")
            # print(f"Cond: {cond}\n")
            # print(f"Cond mask: {cond_mask}\n\n")
        ...
        if self.training:
            if not exists(cond_mask):
                ...
                if self.debug:
                    pass
                    # print(f"Cond mask shape: {cond_mask.shape}")
                    # print(f"Cond mask: {cond_mask}\n\n")
        else:
            if not exists(cond_mask):
        ...
        if cond_drop_prob > 0.0:
            ...
            cond_ids = torch.where(rearrange(cond_drop_mask, "... -> ... 1"), self.null_cond_id, cond_token_ids)
...
class ConditionalFlowMatcherWrapper(Module):
    ...
    def __init__(
        ...
        assert not (
            not self.condition_on_text and exists(text_to_semantic)
        ), "TextToSemantic should not be passed in if not conditioning on text"
        assert not (
            exists(text_to_semantic) and not exists(text_to_semantic.wav2vec)
        ), "the wav2vec module must exist on the TextToSemantic, if being used to condition on text"
        ...
        if self.condition_on_text and (exists(text_to_semantic) or exists(duration_predictor)):
            assert exists(text_to_semantic) ^ exists(
                duration_predictor
            ), "you should use either TextToSemantic from Spear-TTS, or DurationPredictor for the text / phoneme to audio alignment, but not both"
        ...
        self.odeint_kwargs = dict(atol=ode_atol, rtol=ode_rtol, method=torchdiffeq_ode_method)
        ...
        *,
        ...
        if self.condition_on_text:
            if exists(self.text_to_semantic) or exists(semantic_token_ids):
                ...
                # what is cond in this case? audio?
                durations, aligned_phoneme_ids = self.duration_predictor.forward_with_cond_scale(
                    cond=cond, texts=texts, phoneme_ids=phoneme_ids, return_aligned_phoneme_ids=True
                )
            ...
            if exists(cond):
                if exists(self.text_to_semantic):
                    ...
                    cond_target_length = (
                        cond_tokens_seq_len * wav2vec.target_sample_hz / wav2vec.downsample_factor
                    ) / (audio_enc_dec.sampling_rate / audio_enc_dec.downsample_factor)
                    cond_target_length = math.ceil(cond_target_length)

                cond = curtail_or_pad(cond, cond_target_length)
            else:
                cond = torch.zeros(
                    (cond_token_ids.shape[0], cond_target_length, self.voicebox.audio_enc_dec.latent_dim),
                    device=self.device,
                )
        else:
    ...
    def forward(
        ...
        *,
        ...
        if any([input_is_raw_audio, cond_is_raw_audio]):
            assert exists(
                self.voicebox.audio_enc_dec
            ), "audio_enc_dec must be set on VoiceBox to train directly on raw audio"
        ...
        assert self.condition_on_text or not (
            exists(semantic_token_ids) or exists(phoneme_ids)
        ), "semantic or phoneme ids should not be passed in if not conditioning on text"
        ...
        loss = self.voicebox(
            ...
            cond_mask=cond_mask,
            times=times,
            target=flow,  # target is the flow
            self_attn_mask=mask,
            cond_token_ids=cond_token_ids,
